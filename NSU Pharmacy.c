...
int main()
{
    ...
    do
    {
        ...
        printf("Build with love by Tamim Ahmed Rijan\t\t\n");
        printf(".........................................\n");
        printf("#A-Add Medicine\n\n");
        printf("#E-Enter Information about a Medicine\n\n");
        printf("#M-Medicine Information\n\n");
        printf("#T-The stock of Medicine in Store\n\n");
        printf("#C-Change Quantity of Medicine\n\n");
        printf("#D-Delete a Medicine\n\n");
        printf("#P-Purchase Medicine\n\n");
        printf("........Enter First Letter From The List........\n");
        printf("= ");
        scanf("%c",&choice);
        switch(choice)
        {
...
void PurchaseMedicine(int number)
{
    ...
    fflush(stdin);
    {
        printf("Enter You Medicine Name Please\n");
        fflush(stdin);
        gets(name);
        for(i=0; i<number; i++)
        {
            if(strcmp(m[i].medicneName,name)==0)
            {
                ...
                printf("Do you want to purchase %s \nIf Yes Press 1 for exit any other number\n",m[i].medicneName);
                fflush(stdin);
                scanf("%d",&c);
                if(c==1)
                {
                    ...
                    {
                        printf("Total Price to be paid=%d Taka\n",quantity*m[i].price);
                    }
                    else
                    {
...
void EnterInfoAboutMedicine(int number)
{
    ...
    for(i=0; i<number; i++)
    {
        if(strcmp(m[i].medicneName,name)==0)
        {
            flag=1;
            printf("These are the details of Medicine\n");
            printf("Name=%s\nPrice=%d Taka\nAvailable Quantity=%d Piece\nCompany=%s\nMfg Date=%s\nExp Date=%s\n",m[i].medicneName,m[i].price,m[i].quantity,m[i].Company,m[i].Mfg_Date,m[i].Exp_Date);
            if(strcmp(m[i].info,"")!=0)
            {
                printf("Review Already Available!\nIf you want to Add a New Review Enter 1 for exit Any other number\n");
                fflush(stdin);
                scanf("%d",&c);
            }
            else
...
void AddMedicineinStore(int number,struct Medicine m[])
{
    ...
    {
        fprintf(log,"Medicine ID: %d",m[number].id);
        fprintf(log,"Medicine Name: %s",m[number].medicneName);
        fprintf(log,"Company Name: %s",m[number].Company);
       fprintf(log,"Manufactured Date:%s",m[number].Exp_Date);
       fprintf(log,"Expiry Date: %s",m[number].Exp_Date);
       fprintf(log,"Quantity: %d",m[number].quantity);
       fprintf(log,"Price: %d",m[number].price);
    }
    fclose(log);
    printf("Medicine with id %d Added Successfully\n",m[number].id);
    printf("Press 1 to go main menu for exit press any number\n",m[number].id);
}
void DeleteMedicineStore(int number)
{
    int id,i,flag=0,num;
    ...
    for(i=0; i<number; i++)
    {
        if(m[i].id==id && m[i].id!=0)
        {
            do
            {
                ...
                printf("Press 1 to change other details for exit press any number\n");
                fflush(stdin);
                scanf("%d",&c);
            }
            while(c==1);
